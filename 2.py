...
class Taobao_spider():
    ...
    def parse_page(self, text):
        ...
        action.move_to_element(self.driver.find_element_by_partial_link_text('立即购买')).perform()
        ...
        for key_demo in dict_new["propertyPics"].keys():
            ...
            if key_demo != "default":
                ...
                # js实现
                jquery = """$('[data-value="%s"]').children[0].trigger('click')""" % key
                print(jquery)

                if self.driver.find_element_by_xpath('//li[@data-value="%s"]' % key).get_attribute(
                        'class') == 'tb-out-of-stock':
